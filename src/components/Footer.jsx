  ...
  return (
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col bg-primary`}>
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]">
        <p className="font-poppins font-normal text-[18px] text-center text-white">Copyright {'\u00A9'} 2021 HooBank. All Rights Reserved.</p>
         <div className="flex flex-row md:mt-0 mt-6">
          {
            socialMedia.map((social,index)=>(
             <img key={social.id} src={social.icon} alt="socials" className={`w-[21px] h-[21px] object-contain cursor-pointer  hover:text-secondary ${index !== socialMedia.length-1 ? 'mr-6' : 'mr-0'}`}/>
            ))
          }
         </div>
      </div>
